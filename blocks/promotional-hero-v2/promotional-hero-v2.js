...
function readPromotionalConfig(block, sectionData, suffix, fallback) {
  const key = `promoherov2${suffix}`;
  const dataKey = `dataPromoherov2${suffix}`;
...
function warnOnInvalidConfig(name, rawValue, normalizedValue, fallback) {
  ...
  if (raw !== normalized && normalized === fallbackValue) {
    console.warn(`promotional-hero-v2: invalid ${name} "${rawValue}". Using "${fallback}".`);
  }
}

function warnOnNoOpConfig(name, rawValue, reason) {
  if (!rawValue || !rawValue.toString().trim()) return;
  console.warn(`promotional-hero-v2: ${name} "${rawValue}" has no effect. ${reason}`);
}
...
function createDescription(cell, config = {}) {
  ...
  wrapper.className = 'promotional-hero-v2-description';
  ...
  if (config.descriptionMaxLines && config.descriptionMaxLines !== 'none') {
    wrapper.style.setProperty('--promotional-hero-v2-description-max-lines', config.descriptionMaxLines);
  }
...
function buildCard(row, index, config) {
  ...
  card.className = 'promotional-hero-v2-card';

  const imageData = extractImageSource(imageCell);
  if (imageData?.src) {
    const media = document.createElement('div');
    media.className = 'promotional-hero-v2-image';
    ...
    console.warn('promotional-hero-v2: row is missing a valid image source.');
  }

  const content = document.createElement('div');
  content.className = 'promotional-hero-v2-whitebox';
  ...
  button.className = 'promotional-hero-v2-button';
  ...
  if (safeHref) {
    ...
    if (buttonData.href && buttonData.href !== '#') {
      console.warn(`promotional-hero-v2: blocked unsafe CTA URL "${buttonData.href}".`);
    }
  }
...
export default function decorate(block) {
  // Extract key-value config rows authored in the block table
  [...block.children].forEach((row) => {
    ...
    if (cells.length === 2) {
      ...
      if (!value) return;
      if (key === 'bgcolor') {
        block.dataset.promoherov2Bgcolor = value;
        row.remove();
      } else if (key === 'text-color') {
        block.dataset.promoherov2Contenttext = value;
        row.remove();
      } else if (key === 'columns') {
        block.dataset.promoherov2Columns = value;
        row.remove();
      } else if (key === 'cta-button-color') {
        block.dataset.promoherov2Btnfill = value;
        block.dataset.promoherov2Btncolor = value;
        row.remove();
      } else if (key === 'cta-text-color') {
        block.dataset.promoherov2Btntext = value;
        row.remove();
      }
    }
  });

  if (typeof block._promotionalHeroV2Cleanup === 'function') {
    block._promotionalHeroV2Cleanup();
    delete block._promotionalHeroV2Cleanup;
  }
  ...
  warnOnInvalidConfig('promoherov2-align', raw.align, layoutConfig.align, DEFAULTS.align);
  warnOnInvalidConfig('promoherov2-columns', raw.columns, layoutConfig.columns, DEFAULTS.columns);
  warnOnInvalidConfig('promoherov2-cardwidth', raw.cardWidth, layoutConfig.cardWidth, DEFAULTS.cardWidth);
  warnOnInvalidConfig('promoherov2-cardheight', raw.cardHeight, layoutConfig.cardHeight, DEFAULTS.cardHeight);
  warnOnInvalidConfig('promoherov2-imageratio', raw.imageRatio, layoutConfig.imageRatio, DEFAULTS.imageRatio);
  warnOnInvalidConfig('promoherov2-imagefit', raw.imageFit, layoutConfig.imageFit, DEFAULTS.imageFit);
  warnOnInvalidConfig('promoherov2-gap', raw.gap, layoutConfig.gap, DEFAULTS.gap);
  warnOnInvalidConfig('promoherov2-density', raw.density, layoutConfig.density, DEFAULTS.density);
  warnOnInvalidConfig('promoherov2-contentstyle', raw.contentStyle, layoutConfig.contentStyle, DEFAULTS.contentStyle);
  warnOnInvalidConfig('promoherov2-contentalign', raw.contentAlign, layoutConfig.contentAlign, DEFAULTS.contentAlign);
  warnOnInvalidConfig('promoherov2-panelwidth', raw.panelWidth, layoutConfig.panelWidth, DEFAULTS.panelWidth);
  warnOnInvalidConfig('promoherov2-paneloffset', raw.panelOffset, layoutConfig.panelOffset, DEFAULTS.panelOffset);
  warnOnInvalidConfig('promoherov2-gridmaxwidth', raw.gridMaxWidth, layoutConfig.gridMaxWidth, DEFAULTS.gridMaxWidth);
  warnOnInvalidConfig('promoherov2-mobilelayout', raw.mobileLayout, layoutConfig.mobileLayout, DEFAULTS.mobileLayout);
  warnOnInvalidConfig('promoherov2-descmaxlines', raw.descriptionMaxLines, layoutConfig.descriptionMaxLines, DEFAULTS.descriptionMaxLines);
  warnOnInvalidConfig('promoherov2-descstyle', raw.descriptionStyle, layoutConfig.descriptionStyle, DEFAULTS.descriptionStyle);
  warnOnInvalidConfig('promoherov2-width', raw.width, layoutConfig.width, DEFAULTS.width);
  warnOnInvalidConfig('promoherov2-layout', raw.layout, layoutConfig.layout, DEFAULTS.layout);
  warnOnInvalidConfig('promoherov2-contentborderstyle', raw.contentBorderStyle, styleConfig.contentBorderStyle, DEFAULTS.contentBorderStyle);
  warnOnInvalidConfig('promoherov2-contentshadow', raw.contentShadow, styleConfig.contentShadow, DEFAULTS.contentShadow);
  warnOnInvalidConfig('promoherov2-contentradius', raw.contentRadius, styleConfig.contentRadius, DEFAULTS.contentRadius);
  warnOnInvalidConfig('promoherov2-cardborderstyle', raw.cardBorderStyle, styleConfig.cardBorderStyle, DEFAULTS.cardBorderStyle);
  warnOnInvalidConfig('promoherov2-cardborderwidth', raw.cardBorderWidth, styleConfig.cardBorderWidth, DEFAULTS.cardBorderWidth);
  warnOnInvalidConfig('promoherov2-btnstyle', raw.buttonStyle, styleConfig.buttonStyle, DEFAULTS.buttonStyle);
  warnOnInvalidConfig('promoherov2-btncorner', raw.buttonCorner, styleConfig.buttonCorner || '', DEFAULTS.buttonCorner);
  warnOnInvalidConfig('promoherov2-btnwidth', raw.buttonWidth, structureConfig.buttonWidth, DEFAULTS.buttonWidth);
  warnOnInvalidConfig('promoherov2-btncase', raw.buttonTextTransform, styleConfig.buttonTextTransform, DEFAULTS.buttonTextTransform);
  warnOnInvalidConfig('promoherov2-btnborder', raw.buttonBorderWidth, styleConfig.buttonBorderWidth, DEFAULTS.buttonBorderWidth);
  warnOnInvalidConfig('promoherov2-btnshadow', raw.buttonShadow, styleConfig.buttonShadow, DEFAULTS.buttonShadow);
  warnOnInvalidConfig('promoherov2-btnsize', raw.buttonFontSize, styleConfig.buttonFontSize, DEFAULTS.buttonFontSize);
  warnOnInvalidConfig('promoherov2-btnweight', raw.buttonFontWeight, styleConfig.buttonFontWeight, DEFAULTS.buttonFontWeight);
  warnOnInvalidConfig('promoherov2-cardradius', raw.cardRadius, styleConfig.cardRadius, DEFAULTS.cardRadius);
  warnOnInvalidConfig('promoherov2-imageshadow', raw.imageShadow, styleConfig.imageShadow, DEFAULTS.imageShadow);
  warnOnInvalidConfig('promoherov2-contentbg', raw.contentBgColor, colorConfig.contentBgColor, DEFAULTS.contentBgColor);
  warnOnInvalidConfig('promoherov2-contenttext', raw.contentTextColor, colorConfig.contentTextColor, DEFAULTS.contentTextColor);
  warnOnInvalidConfig('promoherov2-contentbordercolor', raw.contentBorderColor, colorConfig.contentBorderColor, DEFAULTS.contentBorderColor);
  warnOnInvalidConfig('promoherov2-cardbordercolor', raw.cardBorderColor, colorConfig.cardBorderColor, DEFAULTS.cardBorderColor);
  warnOnInvalidConfig('promoherov2-btncolor', raw.buttonBorderColor, colorConfig.buttonBorderColor, DEFAULTS.buttonBorderColor);
  warnOnInvalidConfig('promoherov2-btnfill', raw.buttonFillColor, colorConfig.buttonFillColor, DEFAULTS.buttonFillColor);
  warnOnInvalidConfig('promoherov2-btntext', raw.buttonTextColor, colorConfig.buttonTextColor, DEFAULTS.buttonTextColor);
  warnOnInvalidConfig('promoherov2-imageoverlaycolor', raw.imageOverlayColor, colorConfig.imageOverlayColor, DEFAULTS.imageOverlayColor);
  warnOnInvalidConfig('promoherov2-bgcolor', raw.backgroundColor, colorConfig.backgroundColor, DEFAULTS.backgroundColor);
  warnOnInvalidConfig('promoherov2-imageposition', raw.imagePosition, layoutConfig.imagePosition, DEFAULTS.imagePosition);
  warnOnInvalidConfig('promoherov2-hovereffect', raw.hoverEffect, motionConfig.hoverEffect, DEFAULTS.hoverEffect);
  warnOnInvalidConfig('promoherov2-imageoverlay', raw.imageOverlay, motionConfig.imageOverlay, DEFAULTS.imageOverlay);
  warnOnInvalidConfig('promoherov2-btnhovermotion', raw.buttonHoverMotion, motionConfig.buttonHoverMotion, DEFAULTS.buttonHoverMotion);
  warnOnInvalidConfig('promoherov2-btnhovercolor', raw.buttonHoverColor, motionConfig.buttonHoverColor, DEFAULTS.buttonHoverColor);

  if (styleConfig.buttonStyle === 'link') {
    if (hasExplicitConfig.buttonCorner) {
      warnOnNoOpConfig(
        'promoherov2-btncorner',
        raw.buttonCorner,
  ...
  const applyResponsiveLayout = () => {
    ...
    block.style.setProperty('--promotional-hero-v2-linear-max-cols', `${linearColumnCount}`);
    block.style.setProperty('--promotional-hero-v2-layout-scale', layoutScale);
  };
  ...
  block.style.setProperty('--promotional-hero-v2-button-bg', buttonFillColorToken);
  block.style.setProperty('--promotional-hero-v2-button-border', buttonBorderColorToken);
  block.style.setProperty('--promotional-hero-v2-button-text', buttonTextColorToken);
  block.style.setProperty('--promotional-hero-v2-button-hover-bg', buttonHoverBgColorToken);
  block.style.setProperty('--promotional-hero-v2-button-hover-border', buttonHoverBorderColorToken);
  block.style.setProperty('--promotional-hero-v2-button-hover-text', buttonTextColorToken);
  block.style.setProperty('--promotional-hero-v2-content-bg', contentBgColorToken);
  block.style.setProperty('--promotional-hero-v2-content-color', contentTextColorToken);
  block.style.setProperty('--promotional-hero-v2-content-border-color', contentBorderColorToken);
  block.style.setProperty('--promotional-hero-v2-card-border-color', cardBorderColorToken);
  block.style.setProperty('--promotional-hero-v2-image-overlay-color', imageOverlayColorToken);
  block.style.setProperty('--promotional-hero-v2-card-border-width', `${styleConfig.cardBorderWidth}px`);
  block.style.setProperty('--promotional-hero-v2-button-border-width', `${styleConfig.buttonBorderWidth}px`);
  block.style.setProperty('--promotional-hero-v2-button-font-weight', styleConfig.buttonFontWeight);
  block.style.setProperty('--promotional-hero-v2-background', backgroundColorToken);
  applyResponsiveLayout();

  const container = document.createElement('div');
  container.className = 'promotional-hero-v2-container';
  ...
  const firstImage = block.querySelector('.promotional-hero-v2-image img');
  if (firstImage) {
